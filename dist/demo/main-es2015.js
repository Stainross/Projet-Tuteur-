...
let ListeComponent = class ListeComponent {
    ...
    Supprimer(product) {
        return Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"])(this, void 0, void 0, function* () {
            ...
            console.log(product.name + " : " + (yield this.test.getProductData(data[1]['data']['barcode'])).name);
            for (let key in data) {
                console.log("test");
                ...
                if (data[key]['data']['idfamille'] == this.appc.idfamille && (yield this.test.getProductData(data[key]['data']['barcode'])).name == product.name) {
                    ...
                    const data2 = this.http.delete('http://localhost:3000/api/listes/' + data[key]['id'], {}).subscribe({
                        error: error => {
                            console.error('There was an error!', error);
                        }
                    });
